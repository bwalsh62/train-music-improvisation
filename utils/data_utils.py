...
def generate_music(inference_model, corpus = corpus, abstract_grammars = abstract_grammars, tones = tones, tones_indices = tones_indices, 
                   indices_tones = indices_tones, T_y = 10, max_tries = 1000, diversity = 0.5, bpm = 130, song_fname = "my_midi"):
    """
    ...
    Arguments:
    model -- Keras model Instance, output of lstm_model()
    ...
    # (not used?) Tx -- integer, number of time-steps used at training time
    # (not used? but should be?) temperature -- scalar value, defines how conservative/creative the model is when generating music
    # (not used or mentioned, but should be?) diversity -- ...
    ...
    for i in range(1, num_chords):
        ...
        # Consider "A" (approach) and "X" (arbitrary) as "C" (chord) tones. It is a common choice.
    ...
    # Initialize tempo of the output stream with default 130 bit per minute
    ...
    mf.open("output/"+song_fname+".midi", 'wb')
    mf.write()
    print("Your generated music is saved in output/"+song_fname+".midi")
    mf.close()
